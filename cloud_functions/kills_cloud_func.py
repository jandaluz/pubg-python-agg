...
def parquets_to_df(client, bucket, blob_list):
    ...
    for blob in blob_list:
        ...
        if 'kill_distance' in _df.columns and 'kill_type' in _df.columns:
            print(_df.shape)
            _df = _df[_df.weapon.notnull()]
            df = df.append(_df[COMMON_COLUMNS + KILL_KNOCK_COLUMNS])
        elif 'event' in _df.columns:
            print(_df.shape)
            _df = _df[_df.event.str.contains("KILL") | _df.event.str.contains("KNOCK")]
            df = df.append(_df[COMMON_COLUMNS, KILL_KNOCK_COLUMNS])
