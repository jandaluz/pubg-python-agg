...
def zip_and_upload_csv(dataframe, map_name, bucket):
    ...
    if map_df.shape[0] > 0:
        with tempfile.TemporaryDirectory() as tmp_dir:
            csv_string = map_df.to_csv().encode('utf-8')
            csv_bytes = io.BytesIO(csv_string)
            blob = bucket.blob(f"landings/{map_name}.csv")
            blob.upload_from_file(csv_bytes)
            
            gzip_bytes = io.BytesIO(gzip.compress(csv_string))
            blob = bucket.blob(f"landings/compressed/{map_name}.csv")
